 ...
 * We record the call result, optionally link it to an existing contractor
 * application, and send an SMS upload link so the contractor can submit
 * their credentials — all before a human at NGS gets involved.
 */
...
export const Route = createFileRoute("/api/public/webhook/ghl-voice-prequal")({
  async loader({ request }) {
    ...
    if (!rawPhone) return json({ error: "phone required" }, { status: 400 });
    ...
    // Try to match to an existing contractor application by phone
    ...
    // Record the voice pre-qual call
    await supabaseAdmin.from("voice_prequal_calls").insert({
      contractor_id: existing?.id ?? null,
      phone,
      ghl_contact_id: ghlContactId,
      call_disposition: ["qualified","not_qualified","no_answer","callback"].includes(disposition)
        ? disposition
        : "unknown",
    ...
    // Only follow up if call was answered and contractor was qualified or unknown
    if (disposition === "not_qualified") {
      return json({ received: true, action: "none", reason: "not_qualified" });
    }

    // Send upload-link SMS so contractor can submit credentials without NGS involvement
    ...
    const smsBody =
      ...
      `please upload your license, insurance, and bonding documents here — it takes under 5 minutes: ` +
    ...
    try {
      ...
      await supabaseAdmin
        ...
        .order("created_at", { ascending: false })
        .limit(1);
    } catch { /* SMS failure should not break the webhook response */ }

    return json({ received: true, action: "sms_sent", sms_sent: smsSent, contractor_id: existing?.id ?? null });
  },
});
