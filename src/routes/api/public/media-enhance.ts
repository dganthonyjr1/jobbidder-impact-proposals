...
export const Route = createFileRoute("/api/public/media-enhance")({
  server: {
    handlers: {
      POST: async ({ request }) => {
        try {
          ...
          const provider =
            data.provider || PhotoEnhancer.recommendProvider(data.enhancementType as EnhancementType);
